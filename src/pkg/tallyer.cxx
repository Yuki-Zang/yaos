...
void TallyerClient::HandleTally(std::shared_ptr<NetworkDriver> network_driver,
  ...
  // Step 2: receive a vote from the user
  // Receive the vote msg
  VoterToTallyer_Vote_Message v2t_vote_msg;
  auto[params_v2t_vote, ifFromVoter] = crypto_driver->decrypt_and_verify(AES_key, HMAC_key, network_driver->read());
  if (!ifFromVoter){
    network_driver->disconnect();
    throw std::runtime_error("Voter-Tallyer HandleTally: Not from Voter!");
  }

  v2t_vote_msg.deserialize(params_v2t_vote);
  Vote_Ciphertext vote_cipher = v2t_vote_msg.vote;
  CryptoPP::Integer unblinded_sig = v2t_vote_msg.unblinded_signature;
  VoteZKP_Struct vote_zkp = v2t_vote_msg.zkp;

  // verify
  bool ifVoted = this->db_driver->vote_exists(vote_cipher);
  std::cout << "if vote existed:" << ifVoted << std::endl;
  if (ifVoted){
    network_driver->disconnect();
    throw std::runtime_error("Duplicate vote, abort!");
  }
  bool ifSigOk = crypto_driver->RSA_BLIND_verify(this->RSA_registrar_verification_key, vote_cipher, unblinded_sig);
  std::cout << "verification result:" << ifSigOk << std::endl;
  if (!ifSigOk){
    network_driver->disconnect();
    throw std::runtime_error("RSA blind verification failed, abort!");
  }
  bool ifZkpOk = ElectionClient::VerifyVoteZKP(std::make_pair(vote_cipher, vote_zkp), this->EG_arbiter_public_key);
  std::cout << "zkp verification:" << ifZkpOk << std::endl;
  if (!ifZkpOk){
    network_driver->disconnect();
    throw std::runtime_error("Vote zkp verification failed, abort!");
  }

  // Step 3: Sign the vote
  std::vector<unsigned char> concat_to_be_signed = concat_vote_zkp_and_signature(vote_cipher, vote_zkp, unblinded_sig);
  std::string tallyerSign = crypto_driver->RSA_sign(this->RSA_tallyer_signing_key, concat_to_be_signed);

  //construct the vote record and publish it
  TallyerToWorld_Vote_Message t2w_vote;
  t2w_vote.vote = vote_cipher;
  t2w_vote.zkp = vote_zkp;
  t2w_vote.unblinded_signature = unblinded_sig;
  t2w_vote.tallyer_signature = tallyerSign;
  this->db_driver->insert_vote(t2w_vote);
